...
DELETE DIET_REQUESTS, WORKOUT_REQUESTS, NUTRITIONIST_COLLABORATIONS, TRAINER_COLLABORATIONS, NUTRITIONISTS, ATHLETES, TRAINERS, USERS FROM DIET_REQUESTS, WORKOUT_REQUESTS, NUTRITIONIST_COLLABORATIONS, TRAINER_COLLABORATIONS, NUTRITIONISTS, ATHLETES, TRAINERS, USERS;

INSERT INTO TRAINER_COLLABORATIONS (ATHLETE_ID, TRAINER_ID, INIT_DATE, STATUS) VALUES (1, 3, '2000-11-04', true)

SELECT TRAINER_ID, NAME, SURNAME, TITLE, DESCRIPTION
FROM (
    SELECT  T2.TRAINER_ID,  T2.TITLE,  T2.DESCRIPTION 
    FROM (
        SELECT TRAINER_ID AS TRAINER_ID
        FROM TRAINERS
        EXCEPT
        SELECT TRAINER_ID AS TRAINER_ID
        FROM TRAINER_COLLABORATIONS 
        WHERE ATHLETE_ID LIKE ?
        ) AS T1 
    JOIN 
    TRAINERS AS T2 
    WHERE T1.TRAINER_ID LIKE T2.TRAINER_ID
) AS TT1 
JOIN 
USERS AS TT2
WHERE TT1.TRAINER_ID LIKE TT2.USER_ID;
